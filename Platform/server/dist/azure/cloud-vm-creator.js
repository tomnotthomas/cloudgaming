...
async function main(vmName, location) {
...
async function createNetworkInterface(networkClient, resourceGroupName, location, interfaceName, networkName, subnetName, publicIPId, virtualMachineName) {
  const nicParameters = {
    ...
    ipConfigurations: [{
      name: "MyIpConfig",
      subnet: {
        id: `/subscriptions/${process.env.AZURE_SUBSCRIPTION_ID}/resourceGroups/${resourceGroupName}/providers/Microsoft.Network/virtualNetworks/${networkName}/subnets/${subnetName}`
      },
      publicIPAddress: {
        id: publicIPId
      }
