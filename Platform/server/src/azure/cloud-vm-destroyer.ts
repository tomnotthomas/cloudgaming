...
export async function virtualMachines_delete(resourceGroupName, virtualMachineName) {
  // Delete the VM
  await computeClient.virtualMachines.beginDeleteAndWait(resourceGroupName, virtualMachineName)
    .then(response => console.log(`VM deleted: ${virtualMachineName}`, response))
    .catch(error => console.error(`Error deleting VM: ${virtualMachineName}`, error));
